...
// SignalR imports
'use client';
import { createChatHubConnection, registerChatHubListeners, sendTypingIndicator, markMessagesAsRead, joinChat, leaveChat } from '@/lib/signalr/chatHub';
...
const BigChatWindow = () => {
  ...
  const [readMessages, setReadMessages] = useState<{ [chatId: string]: string[] }>({});
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [chatStarted, setChatStarted] = useState<any>(null);
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const [chatUpdated, setChatUpdated] = useState<any>(null);
  ...
  const handleTyping = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (connection && selectedChat?.id) {
      sendTypingIndicator(connection, selectedChat.id || "", true);
      setTimeout(() => {
        sendTypingIndicator(connection, selectedChat.id || "", false);
      }, 1500);
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (response?.isSuccess) {
        ...
        setSelectedFiles([]);
        refetch && refetch();
        // Send typing indicator false after sending
        if (connection && selectedChat?.id) {
          sendTypingIndicator(connection, selectedChat.id || "", false);
        }
      } else {
  ...
  useEffect(() => {
        ...
        registerChatHubListeners(conn, {
          ...
          onMessagesRead: (payload) => {
            setReadMessages((prev) => ({
              ...prev,
              [payload.chatId]: Array.from(new Set([...(prev[payload.chatId] || []), ...payload.messageIds])),
            }));
          },
          onChatStarted: (payload) => {
            setChatStarted(payload);
            refetchchatList && refetchchatList();
          },
          onChatUpdated: (payload) => {
            setChatUpdated(payload);
            refetchchatList && refetchchatList();
          },
        });
      })
    ...
    return () => {
      leaveChat(conn, selectedChat.id || "");
      conn.stop();
    };
  ...
  useEffect(() => {
    if (connection && selectedChat?.id && messageList.length > 0) {
      const unreadIds = messageList
        .filter((msg) => msg.userId !== userId && !(readMessages[selectedChat?.id || ""] || []).includes(msg.id || ""))
        .map((msg) => msg.id)
        .filter((id): id is string => typeof id === 'string');
      if (unreadIds.length > 0) {
        markMessagesAsRead(connection, selectedChat.id || "", unreadIds);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [messageList, connection, selectedChat?.id]);
          ...
          <>
            ...
            <div className="flex-shrink-0 border-t border-gray-200 bg-white">
              ...
              {selectedChat && typingUsers[selectedChat.id || '']?.length > 0 && (
                <div className="px-4 py-2 text-xs text-blue-500 font-medium">
                  {typingUsers[selectedChat.id || ''].length === 1
                    ? `${typingUsers[selectedChat.id || ''][0]} is typing...`
                    : `${typingUsers[selectedChat.id || ''].join(', ')} are typing...`}
                </div>
              )}
              {/* Online status */}
              {selectedChat && onlineUsers.includes(selectedChat.sender?.id || '') && (
                <div className="px-4 py-2 text-xs text-green-500 font-medium">
                  Online
                </div>
              )}
            </div>
